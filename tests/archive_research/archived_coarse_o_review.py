# Unit-тесты review-хелперов для rare-tail `O` класса.
...
def _build_o_source_df() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "source_id": [1, 2, 3, 4],
            "spectral_class": ["O", "O", "O", "O"],
            "quality_state": ["pass", "pass", "unknown", "reject"],
            "quality_reason": [
                "pass",
                "pass",
                "high_ruwe",
            ...
            "teff_gspphot": [32000.0, 31000.0, 29500.0, 29000.0],
            "logg_gspphot": [4.0, 4.1, 4.0, 3.9],
            "mh_gspphot": [0.0, -0.1, -0.2, -0.2],
            "bp_rp": [-0.25, -0.22, -0.18, -0.15],
            "parallax": [1.9, 2.1, 1.5, 1.3],
            "parallax_over_error": [16.0, 15.0, 4.0, 2.0],
            "ruwe": [1.0, 1.1, 1.8, 1.2],
            "radius_flame": [8.2, 7.7, 7.1, pd.NA],
...
def _build_final_decision_artifact(tmp_path: Path) -> Path:
    paths = save_final_decision_artifacts(
        ...
        final_decision_df=pd.DataFrame(
            {
                "source_id": [1, 2, 3, 4],
                "final_domain_state": ["id", "id", "unknown", "unknown"],
                "final_quality_state": ["pass", "pass", "unknown", "reject"],
                "final_coarse_class": ["B", "O", pd.NA, pd.NA],
                "final_coarse_confidence": [0.91, 0.66, pd.NA, pd.NA],
                "final_refinement_label": [pd.NA, pd.NA, pd.NA, pd.NA],
                ...
                "final_refinement_confidence": [pd.NA, pd.NA, pd.NA, pd.NA],
...
def test_coarse_o_review_bundle_covers_source_scoring_and_final_outcomes(
    tmp_path: Path,
) -> None:
    ...
    bundle = build_coarse_o_review_bundle(
        source_df,
        coarse_model_run_dir=model_paths.run_dir,
        final_decision_run_dir=final_run_dir,
    )

    quality_summary_df = build_o_source_quality_summary_frame(bundle.source_df)
    quality_reason_df = build_o_source_quality_reason_frame(bundle.source_df)
    scored_prediction_df = build_o_scored_prediction_frame(bundle.scored_pass_df)
    high_confidence_preview_df = build_o_high_confidence_non_o_preview_frame(
        bundle.scored_pass_df
    )
    predicted_physics_df = build_o_predicted_physics_summary_frame(bundle.scored_pass_df)
    final_outcome_df = build_o_final_outcome_distribution_frame(bundle.final_o_df)
    final_coarse_df = build_o_final_coarse_distribution_frame(bundle.final_o_df)
    final_reason_df = build_o_final_reason_frame(bundle.final_o_df)

    assert _require_int_scalar(quality_summary_df["n_rows"].sum()) == 4
    assert "pass" in set(quality_summary_df["quality_state"])
    assert "high_ruwe" in set(quality_reason_df["quality_reason"])
    assert int(bundle.pass_source_df.shape[0]) == 2
    assert int(bundle.scored_pass_df.shape[0]) == 2
    assert "coarse_predicted_label" in bundle.scored_pass_df.columns
    assert "coarse_probability_max" in bundle.scored_pass_df.columns
    assert _require_int_scalar(scored_prediction_df["n_rows"].sum()) == 2
    assert int(high_confidence_preview_df.shape[0]) <= 2
    assert "median_teff_gspphot" in predicted_physics_df.columns
    assert "id" in set(final_outcome_df["final_domain_state"])
    assert "B" in set(final_coarse_df["final_coarse_class"])
    assert "quality_reject" in set(final_reason_df["final_decision_reason"])
