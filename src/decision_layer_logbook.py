...
После запуска будет создан следующий файл:
    experiments/Логи калибровки decision_layer/iteration_XXX.md
"""
...
def list_iteration_files(logbook_dir: Path) -> list[Path]:
    ...
    for path in sorted(logbook_dir.iterdir()):
        if path.is_file() and ITERATION_PATTERN.match(path.name):
            files.append(path)
    return files
...
def next_iteration_number(logbook_dir: Path) -> int:
    ...
    max_number = 0
    for path in list_iteration_files(logbook_dir):
