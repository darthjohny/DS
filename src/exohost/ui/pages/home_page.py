...
def render_home_page() -> None:
    ...
    st.info(
        ...
        "список кандидатов без запуска notebook."
    )
    st.markdown(
        "Главный прикладной результат проекта — воспроизводимый shortlist целей "
    ...
    try:
        ...
        st.error(f"Не удалось загрузить latest `run_dir`: {exc}")
        return
    ...
    st.markdown(
        "- Страница `Метрики` показывает benchmark-сводку по основным model layers.\n"
        "- Страница `Запуск` открывает готовый `run_dir` и итоговые распределения.\n"
        "- Страница `Объект` показывает маршрут по pipeline и физические параметры одной звезды.\n"
