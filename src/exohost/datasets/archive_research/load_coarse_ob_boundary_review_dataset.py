# Narrow loader для `O/B` boundary-review source из quality-gated relation.
...
def build_coarse_ob_boundary_review_query(
    relation_name: str,
    ...
    *,
    ...
    # Собираем SQL для narrow `O/B` boundary source.
...
def load_coarse_ob_boundary_review_dataset(
    engine: Engine,
    *,
    ...
    # Загружаем narrow `O/B` boundary source из quality-gated relation.
    if not relation_exists(engine, contract.relation_name, validate_identifiers=True):
        raise RuntimeError(
            f"Coarse O/B boundary review source does not exist: {contract.relation_name}"
        )
    ...
    if missing_columns:
        ...
        raise RuntimeError(
            "Coarse O/B boundary review source is missing required columns: "
