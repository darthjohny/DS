# Файл `load_gaia_mk_refinement_training_dataset.py` слоя `datasets`.
#
...
def build_gaia_mk_refinement_training_query(
    relation_name: str,
    selected_columns: tuple[str, ...],
    *,
    limit: int | None = None,
) -> str:
    # Собираем явный SQL для refinement training view.
    ...
    WHERE source_id IS NOT NULL
      AND spectral_class IN ({supported_classes_sql})
...
def load_gaia_mk_refinement_training_dataset(
    engine: Engine,
    *,
    contract: DatasetContract = GAIA_MK_REFINEMENT_TRAINING_CONTRACT,
    limit: int | None = None,
) -> pd.DataFrame:
    # Загружаем refinement training view по зафиксированному contract.
    if not relation_exists(engine, contract.relation_name, validate_identifiers=True):
        raise RuntimeError(
            f"Refinement training source does not exist: {contract.relation_name}"
        )
    ...
    if missing_columns:
        ...
        raise RuntimeError(
            "Refinement training source is missing required columns: "
            f"{missing_columns_sql}"
        )

    selected_columns = select_contract_columns(contract, available_columns)
    query = build_gaia_mk_refinement_training_query(
        contract.relation_name,
        selected_columns,
        limit=limit,
    )
