...
- текущий источник пригоден для первой рабочей версии задачи поиска
  host-подобных объектов;
...
- `parallax`
- `parallax_over_error`
- координатные поля на небе, если они нужны следующему слою
...
2. слой ранжирования и приоритета отдельно использует:
   - `class priority`
...
- основным радиусом считается `radius_flame`;
