# Контракт импортных колонок MK-волны
...
- без координат нет воспроизводимого кроссматча;
- без сырого `SpType` нет нормального label parsing;
- без сырого `SpType` нет нормального разбора метки;
...
- если старый совместимый загрузчик требует `radius_gspphot`, этот псевдоним
  строится отдельно поверх основного `radius_flame`.
...
- дополнительные фотометрические и служебные поля, если они нужны для
  практического слоя наблюдаемости.
...
## Технические колонки прослеживаемости
...
- без них сложно воспроизводить и объяснять, как объект попал в новую
  обучающую ветку.
...
- план загрузки может прямо ссылаться на этот список.
